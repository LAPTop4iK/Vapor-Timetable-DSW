...
//                    if t == "Brak" || t == "Distance learning" || t == "Sala na zajęcia zdalne" || t == "Zajęcia odwołane" || t.hasPrefix("Uwagi:") {
//                        foundRemarks = t
//                    }
//                }
//            }
...
//
//func isMock(_ req: Request) -> Bool {
//    false
//}
...
//
//        if isMock(req) {
//            return MockFactory.makeAggregate(groupId: gid, from: "2025-10-01", to: to, intervalType: interval)
//        }
//
...
//
//    // /api/feature-flags
//    app.get("api","feature-flags") { req async throws -> FeatureFlagsResponse in
//        if isMock(req) { return MockFactory.featureFlags() }
//        let flags: [String : JSONValue] = [
//            "show_subjects_tab":.bool(false),
//            "show_teachers_tab":.bool(false),
//            "enable_analytics":.bool(false),
//            "show_ads":.bool(false),
//            "enable_push_notifications":.bool(false),
//            "dark_mode_only":.bool(false),
//            "show_debug_menu":.bool(false),
////            "show_subjects_tab":true,
////            "show_teachers_tab":true,
...
//        ]
//
//        req.logger.info("Sended feature flags \(flags) groups")
//        let responce = FeatureFlagsResponse(flags: flags, version: "1.0(1)", updatedAt: Date.timeIntervalSinceReferenceDate.description)
//        return responce
//    }
//
//    app.get("api","feature-parameters") { req async throws -> FeatureParametersResponse in
//        if isMock(req) { return MockFactory.featureParameters() }
//        let parameters: [String : JSONValue] = [
//            ParameterCases.premiumTrialDuration.rawValue: .int(60 * 60 * 24)
//        ]
//
//        req.logger.info("Sended feature parameters \(parameters) groups")
//        let responce = FeatureParametersResponse(parameters: parameters, version: "1.0(1)", updatedAt: Date.timeIntervalSinceReferenceDate.description)
//        return responce
//    }
//
//    app.get("api", "groups", ":groupId", "schedule") { req async throws -> GroupScheduleResponse in
//        guard let gid = req.parameters.get("groupId", as: Int.self) else {
//            throw Abort(.badRequest)
//        }
//
//        // Параметры периода (дефолты — как в aggregate)
//        let from = (try? req.query.get(String.self, at: "from")) ?? "2025-09-06"
//        let to   = (try? req.query.get(String.self, at: "to"))   ?? "2026-02-08"
//        let tRaw = (try? req.query.get(Int.self, at: "type")) ?? 3
//        let interval = IntervalType(rawValue: tRaw) ?? .semester
//
//        if isMock(req) {
//            return MockFactory.makeGroupScheduleOnly(groupId: gid, from: from, to: to, intervalType: interval)
//        }
//
//        // Реальный запрос только к расписанию группы
//        let client = VaporDSWClient(client: req.client)
//        let parser = SwiftSoupScheduleParser(logger: req.logger)
//
//        let gHTML = try await client.groupScheduleHTML(groupId: gid, from: from, to: to, interval: interval)
//        let groupEvents = try parser.parseSchedule(gHTML)
//
...
//
//
//// MockFactory.swift — обновлённые настройки и генерация
//
//
//// ===== Простой детерминированный RNG (для стабильности объёмов per-teacher) =====
